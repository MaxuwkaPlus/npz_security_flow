...
uv run python -m app.cli seed-users
...
`seed-users` печатает пароли учётных записей **один раз** — выпишите их, дальше
они понадобятся на каждом шаге. Записи заводятся по одной на роль:
`operator-1`, `operator-2` (обучаемые), `instructor-1`, `expert-1`, `iso-1`.
...
**Доступ** (вход, роли, учётные записи), **Аудит** (журнал безопасности),
**catalog** (сценарии, установка), **sessions** (сессия, команды, наблюдения,
тревоги, SAGAT, NASA-TLX), **reports** (отчёт).

### Вход: без токена не работает ничего

Проверьте это сразу: разверните **`GET /scenarios`** → **Try it out** →
**Execute** без входа. **Ожидаемо:** `401`, `code: MISSING_TOKEN`. Так и должно
быть — без токена открыты только `/health`, `/ready`, `/auth/login` и
`/auth/guest`.

**`POST /auth/guest`** выдаёт токен тому, кто садится за пульт без учётной
записи. Это не дыра в разграничении, а отдельная роль `guest`: в ответе
`user.permissions` перечислены `session.create`, `session.control_own`,
`session.operate`, `session.read_own`, `report.read_own` — прав ровно на своё
прохождение. Ни `session.control`, ни `session.read_any`, ни `audit.read` там
нет, а назначить роль `guest` учётной записи администратор не может: `POST /users`
с `roles: ["guest"]` отвечает `ROLE_NOT_AVAILABLE`. Отключается режим настройкой
`allow_guest_training=false`.

Теперь войдите: **`POST /auth/login`** (тег **Доступ**), тело
`{"username": "instructor-1", "password": "<пароль из seed-users>"}`.
Скопируйте `access_token` из ответа, нажмите кнопку **Authorize** вверху
страницы `/docs` и вставьте его. С этого момента Swagger подставляет токен во
все запросы.

**Обратите внимание на `user.permissions` в ответе** — это полный список того,
что разрешено роли. У инструктора там есть `session.create` и `session.control`,
но нет `session.operate`.

### Токен меняется по ходу плана

Права разделены, поэтому один токен весь план не покрывает. Держите под рукой
оба пароля и переключайтесь через **Authorize**:

| Шаги | Под кем | Почему |
|---|---|---|
| 1 — каталог | любой | справочники доступны всем вошедшим |
| 2 — создание, старт, пауза, прерывание сессии | `instructor-1` | `session.create`, `session.control` |
| 3–9 — осмотры, команды, диагноз, тревоги, SAGAT, NASA-TLX | `operator-1` | `session.operate`, и только для своей сессии |
| 10+ — отчёт | `instructor-1` | `report.read_any` |

Весь этот план можно пройти и одним гостевым токеном из `POST /auth/guest`,
подставив в `operator_id` его же `username`: у гостя есть и создание, и ведение
своего прохождения, и работа за пультом. Переключение токенов нужно именно для
того, чтобы увидеть разделение полномочий у **именных** учётных записей — на
гостевом токене оно не проверяется, потому что делить не с кем.

Границы гостя стоит проверить отдельно: под гостевым токеном создайте сессию с
чужим `operator_id` (**ожидаемо** `403`), попробуйте запустить чужую сессию
(**ожидаемо** `403`, `required_any_of: ["session.control"]`), откройте
`GET /sessions` (**ожидаемо** только свои) и `GET /users` (**ожидаемо** `403`).

Если получили `403` — почти всегда вы под не тем токеном. В `details.required_any_of`
написано, какого права не хватило.
...
| `<TOKEN>` | 0 `POST /auth/login` | `.access_token` | кнопка **Authorize** | заголовок ко всем запросам |
...
| `ratio: 0.5` | `202`, `status: "rejected"`, `rejection_reason: "value_out_of_range"` — допустимый ввод ограничен 0.20 |
