...
- наблюдает параметры и тревоги;
- выполняет команды;
- явно фиксирует обнаруженное отклонение и предполагаемую причину;
- отвечает на SAGAT-вопросы;
- заполняет NASA-TLX;
- получает свой отчёт.

### Инструктор

- выбирает сценарий и уровень;
- создаёт сессию для оператора;
- запускает, ставит на паузу или прекращает прохождение;
- наблюдает за сессией;
- просматривает детальный отчёт и журнал;
- не изменяет скрытые параметры после запуска.

### Администратор/методист

- управляет версиями установки, сценария, правил тревог и оценки;
- публикует новую конфигурацию;
- не редактирует опубликованную версию задним числом.

Для конкурсного MVP допустима упрощённая авторизация. Роли и проверки доступа всё равно должны находиться в доменном/API-слое, а не только во фронтенде.
